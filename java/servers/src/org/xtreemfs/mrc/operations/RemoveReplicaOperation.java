...
public class RemoveReplicaOperation extends MRCOperation implements XLocSetCoordinatorCallback {
  ...
  @Override public void startRequest(MRCRequest rq) throws Throwable {
    ...
    if (rqArgs.hasFileId()) {
      ...
      if (file == null) {
        throw new UserException(POSIXErrno.POSIX_ERROR_ENOENT, "file \'" + rqArgs.getFileId() + "\' does not exist");
      }
    } else {
    ...
    if (sMan.getSoftlinkTarget(file.getId()) != null) {
      throw new UserException(POSIXErrno.POSIX_ERROR_EINVAL, "file \'" + rqArgs.getFileId() + "\' is a symbolic link");
    }
    ...
    if (ReplicaUpdatePolicies.REPL_UPDATE_PC_NONE.equals(oldXLocList.getReplUpdatePolicy())) {
      throw new UserException(POSIXErrno.POSIX_ERROR_EINVAL, "Replica cannot be removed because the file\'s replication policy is set to \'none\' i.e., " + "the file has only one replica which shouldn\'t be deleted. Delete the whole file instead.");
    }
    int i = 0;
    XLoc replica = null;
    ...
    if (i == oldXLocList.getReplicaCount()) {
      throw new UserException(POSIXErrno.POSIX_ERROR_EINVAL, "OSD \'" + rqArgs.getOsdUuid() + "\' is not head OSD of any replica");
    }
    ...
    if (ReplicaUpdatePolicies.REPL_UPDATE_PC_RONLY.equals(oldXLocList.getReplUpdatePolicy())) {
      ...
      for (int k = 0; k < newXLocList.getReplicaCount(); k++) {
        if (ReplicationFlags.isReplicaComplete(newXLocList.getReplica(k).getReplicationFlags()) || ReplicationFlags.isFullReplica(newXLocList.getReplica(k).getReplicationFlags())) {
          completeOrFullExists = true;
          break;
        }
      }
      if (!completeOrFullExists) {
        throw new UserException(POSIXErrno.POSIX_ERROR_EINVAL, "Could not remove OSD \'" + rqArgs.getOsdUuid() + "\': read-only replication w/ partial replicas requires at " + "least one remaining replica that is full or complete");
      }
    }
    XLocSetCoordinator coordinator = master.getXLocSetCoordinator();
    XLocSetCoordinator.RequestMethod m = coordinator.requestXLocSetChange(fileId, file, oldXLocList, newXLocList, rq, this);
    ...
    int deleteCapTimeout = master.getConfig().getCapabilityTimeout() + (master.getConfig().getFleaseLeaseToMS() / 1000);
    Capability deleteCap = new Capability(idRes.getVolumeId() + ":" + file.getId(), FileAccessManager.NON_POSIX_DELETE, deleteCapTimeout, Integer.MAX_VALUE, ((InetSocketAddress) rq.getRPCRequest().getSenderAddress()).getAddress().getHostAddress(), file.getEpoch(), false, !sMan.getVolumeInfo().isSnapshotsEnabled() ? SnapConfig.SNAP_CONFIG_SNAPS_DISABLED : sMan.getVolumeInfo().isSnapVolume() ? SnapConfig.SNAP_CONFIG_ACCESS_SNAP : SnapConfig.SNAP_CONFIG_ACCESS_CURRENT, sMan.getVolumeInfo().getCreationTime(), master.getConfig().getCapabilitySecret());
    xtreemfs_replica_removeResponse response = xtreemfs_replica_removeResponse.newBuilder().setFileId(fileId).setExpectedXlocsetVersion(newXLocList.getVersion()).setUnlinkXcap(deleteCap.getXCap()).setUnlinkXloc(Converter.xLocListToXLocSet(oldXLocList)).build();
    rq.setResponse(response);
    update.execute();
  }

  @Override public void installXLocSet(String fileId, XLocList xLocList, XLocList oldXLocList) throws Throwable {
    final VolumeManager vMan = master.getVolumeManager();
    final GlobalFileIdResolver idRes = new GlobalFileIdResolver(fileId);
  ...
  @Override public void handleInstallXLocSetError(Throwable error, String fileId, XLocList newXLocList, XLocList prevXLocList) throws Throwable {
    final VolumeManager vMan = master.getVolumeManager();
    final GlobalFileIdResolver idRes = new GlobalFileIdResolver(fileId);
    final StorageManager sMan = vMan.getStorageManager(idRes.getVolumeId());
    final FileMetadata file = sMan.getMetadata(idRes.getLocalFileId());
    ...
    XLocList xLocList = sMan.createXLocList(replicas, prevXLocList.getReplUpdatePolicy(), newXLocList.getVersion() + 1);
    AtomicDBUpdate update = sMan.createAtomicDBUpdate(null, null);
    file.setXLocList(xLocList);
    sMan.setMetadata(file, FileMetadata.RC_METADATA, update);
