...
public class TracingTest {
  @Rule public final TestRule testLog = TestHelper.testLog;
  ...
  @Before public void setUp() throws Exception {
    ...
    env = new TestEnvironment(new TestEnvironment.Services[] { TestEnvironment.Services.TIME_SYNC, TestEnvironment.Services.DIR_SERVICE, TestEnvironment.Services.MRC, TestEnvironment.Services.OSD, TestEnvironment.Services.OSD, TestEnvironment.Services.OSD });
    env.start();
    uc = RPC.UserCredentials.newBuilder().setUsername("service").addGroups("service").build();
    client = ClientFactory.createClient(env.getDIRAddress().getHostName() + ":" + env.getDIRAddress().getPort(), uc, null, new Options());
    client.start(true);
  }
  ...
  @Test public void testFileBasedTracing() throws Exception {
  ...
  private void readWriteFile(Volume volume) throws IOException {
    FileHandle f = volume.openFile(uc, TEST_FILE, GlobalTypes.SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_CREAT.getNumber() | GlobalTypes.SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_WRONLY.getNumber());
  ...
  private Volume createVolume(String tracingPolicy, String tracingPolicyConfig) throws IOException {
    client.createVolume(env.getMRCAddress().getHostName() + ":" + env.getMRCAddress().getPort(), auth, uc, SOURCE_VOLUME);
    Volume sourceVolume = client.openVolume(SOURCE_VOLUME, null, new Options());
    sourceVolume.setXAttr(uc, auth, "/", "xtreemfs.tracing_enabled", "1", MRC.XATTR_FLAGS.XATTR_FLAGS_CREATE);
    if (tracingPolicyConfig != null) {
      sourceVolume.setXAttr(uc, auth, "/", "xtreemfs.tracing_policy_config", tracingPolicyConfig, MRC.XATTR_FLAGS.XATTR_FLAGS_CREATE);
    }
    if (tracingPolicy != null) {
      sourceVolume.setXAttr(uc, auth, "/", "xtreemfs.tracing_policy", tracingPolicy, MRC.XATTR_FLAGS.XATTR_FLAGS_CREATE);
    }
    return sourceVolume;
  }

  @Test public void testSocketBasedTracing() throws Exception {
